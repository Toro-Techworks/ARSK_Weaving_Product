...
export function WeavingUnitList() {
  ...
  const toggleUnitStatus = (row) => {
    ...
    api
      .put(`/weaving-units/${row.id}`, { status: next })
...
function WeavingUnitAddModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = (e) => {
    ...
    setLoading(true);
    api.post('/weaving-units', { ...form, phone: form.phone || null, status: form.status || 'Active' })
...
function WeavingUnitEditModal({ unit, onClose, onSuccess }) {
  ...
  const handleSubmit = (e) => {
    ...
    setSaving(true);
    api.put(`/weaving-units/${unit.id}`, { ...form, phone: form.phone || null, status: form.status || 'Active' })
      ...
      <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-6">
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className={fieldClass}>
              <FormPhoneInput
                id={`weaving-unit-edit-phone-${unit.id}`}
                label="Phone"
                value={form.phone}
                onChange={(next) => setForm({ ...form, phone: next })}
                placeholder="Mobile number (optional)"
